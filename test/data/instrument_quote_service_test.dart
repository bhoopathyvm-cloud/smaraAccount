...
void main() {
  ...
  group('searchIdentifier', () {
    ...
    test(
      'returns candidates with a currency inferred from the suffix',
      () async {
        final service = InstrumentQuoteService(
  ...
  group('stooqCurrencyForSymbol', () {
    test('maps market suffixes to their ISO-4217 currency', () {
      ...
      // NSE (instrument-identifier-assist's national-fallback exchange for
      // India) - missing from this map made Stooq's own NSE quotes always
      // look like a miss, needlessly triggering the Yahoo national
      // fallback even when Stooq's response was perfectly good.
      expect(
        InstrumentQuoteService.stooqCurrencyForSymbol('reliance.ns'),
        'INR',
      );
    });

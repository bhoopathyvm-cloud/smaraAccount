...
- [ ] 1.11 `flutter analyze` clean; full `flutter test` passes unmodified; the full acceptance/integration suite also passes (`flutter test integration_test/acceptance/<file>.dart -d macos`, run once per file — batching multiple files in one invocation breaks app launch on macOS) before this group is marked done (CLAUDE.md Working Conventions: run the acceptance suite after each big refactor, not only at the end of the change).

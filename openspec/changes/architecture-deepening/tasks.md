...
- [ ] 1.1 Create `lib/data/repositories/account_repository.dart` with `AccountRepository({required AppDatabase database})`, moving `createAccountGroup`, `renameAccountGroup`, `archiveAccountGroup`, `unarchiveAccountGroup`, `deleteAccountGroup`, `changeAccountGroupCurrency`, `backfillGroupCurrencies`, `needsCurrencyBackfill`, `reassignFinancialAccountGroup`, `createFinancialAccount`, `renameFinancialAccount`, `archiveFinancialAccount`, `unarchiveFinancialAccount`, `watchFinancialAccounts`, `recordArchivedAccountCloseoutTransfer`, and every private helper only these call (verbatim — no logic changes), out of `LedgerRepository`.
- [ ] 1.2 `recordArchivedAccountCloseoutTransfer` needs `LedgerRepository`'s transfer-posting: take `LedgerRepository` as a constructor dependency (design.md D2) rather than duplicating posting logic.
- [ ] 1.3 Register `ProxyProvider2<AppDatabase, LedgerRepository, AccountRepository>` in `main.dart`.
- [ ] 1.4 Update `AccountManagementViewModel` and `RegisterViewModel` to take `AccountRepository` for the methods this group moved (RegisterViewModel keeps `LedgerRepository` too, for its own core-journal methods).
- [ ] 1.5 Delete the moved methods and their now-unused private helpers from `LedgerRepository`. Confirm no remaining call site references them there.
- [ ] 1.6 `flutter analyze` clean; full `flutter test` passes unmodified.

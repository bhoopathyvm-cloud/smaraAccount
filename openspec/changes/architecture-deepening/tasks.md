...
- [ ] 2.3 Create `lib/data/repositories/ledger_backup_repository.dart` with `LedgerBackupRepository({required AppDatabase database, required LedgerRepository ledgerRepository, SigningKeyService? signingKeyService})`, moving `exportLedgerBackup`, `restoreLedgerBackup`. **Corrected, not a true leaf** (design.md D2): `restoreLedgerBackup` calls `ledgerRepository.currentIdentity()` to compare the device's active identity against the backup's, and constructs its own throwaway `LedgerRepository` (using its own `signingKeyService`, defaulted to `SigningKeyService()` the same way `LedgerRepository` itself defaults one — it cannot reach `LedgerRepository`'s private `_signingKeyService` field) wrapping a temp file to validate the backup before replacing the real database file. Register a `ProxyProvider2<AppDatabase, LedgerRepository, LedgerBackupRepository>`; update `SettingsViewModel`.
- [ ] 2.4 Delete the moved methods from `LedgerRepository`. Confirm no remaining call site references them there.
- [ ] 2.5 `flutter analyze` clean; full `flutter test` passes unmodified; full acceptance/integration suite passes (see 1.11).
...
- [ ] 3.7 `flutter analyze` clean; full `flutter test` passes unmodified; full acceptance/integration suite passes (see 1.11).
...
- [ ] 4.4 `flutter analyze` clean; full `flutter test` passes unmodified; full acceptance/integration suite passes (see 1.11).
...
- [ ] 5.4 `flutter analyze` clean; full `flutter test` passes unmodified; full acceptance/integration suite passes (see 1.11).
...
- [ ] 6.3 Full `flutter test` run, unmodified test files, all green (internal-architecture spec: "Existing test suite passes unmodified"); full acceptance/integration suite passes too (see 1.11).
...
- [ ] 8.8 `flutter analyze` clean; full `flutter test` passes unmodified; full acceptance/integration suite passes (see 1.11).

## 9. Final verification

- [ ] 9.1 Full `flutter test` run across the whole suite, no test file modified for this change; final full acceptance/integration suite run (see 1.11).

...
- `multi-account-support`'s existing account-group reassignment feature gains a constraint: an account can only be reassigned to a group with the same currency as its current group, to avoid retroactively reinterpreting historical balances in a different currency.
- A provisional entry cannot be reversed through the general reversal action while its pending transfer is still unsettled (settling it back to its own source account achieves the same outcome); settlement itself rejects a negative amount, an amount exceeding what was originally sent, an already-settled pending transfer, or a fee category that isn't an active expense category (fee categories are only meaningful when settling back to the source account in the first place).
- Every group total, account balance, and pending-transfer amount on the home overview is labeled with its currency, since an unlabeled figure becomes ambiguous once more than one currency exists.
...
- `multi-account-ledger` (introduced by `multi-account-support`, not yet archived): `recordTransfer` branches into same-currency (unchanged), cross-currency-known-upfront (single entry), and cross-currency-unknown (provisional + later settlement via `foreign-currency-settlement`). Foreign-currency income/expense transactions get the same provisional/settlement treatment when the account's group currency doesn't match the transaction's native currency. The existing group-reassignment feature gains a same-currency restriction, and reversing a transfer's provisional leg while its pending transfer is still unsettled is disallowed (delegated to `foreign-currency-settlement`).
- `accounts-home-overview` (introduced by `multi-account-support`, not yet archived): "Overall Net Position" becomes one net position per currency; adds the "Pending transfers" section and a requirement that every group total, account balance, and pending-transfer amount is labeled with its currency.
...
- New `schemaVersion` bump (4th — after `core-ledger-single-account`'s 1, `ledger-integrity-signing`'s 2, and `multi-account-support`'s 3; confirm the exact number once `multi-account-support` archives), adding `account_groups.currency`, a system "Transfers-in-transit" account, and a new pending-transfer/pending-settlement table.

...
The system SHALL validate active income/expense categories through `AccountChartReader` (or the shared chart seam), not private `_requireActiveCategoryOfType` / `_requireActiveExpenseCategory` copies in posting and investment. Exception type per caller MAY stay distinct via an explicit policy argument.

#### Scenario: Posting and investment share the chart lookup

...
- [x] 2.8 New, found live in Play Console's Store listing screen (2026-09-14): the app icon field's own text specifies "512 px by 512 px" PNG/JPEG, up to 1MB — the existing `assets/branding/app_icon.png` is 1024x1024. Generated `play-store-icon-512.png` by resizing the same approved artwork (no new design), 512x512, 201KB
...
- [x] 5.1 Update `app-store-launch-readiness`'s section 5 to point at this change instead of carrying its own unchecked copies of these tasks, once this change's tasks are complete

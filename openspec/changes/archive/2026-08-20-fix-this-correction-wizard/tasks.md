...
- [x] 2.2 `CorrectionViewModel` (lib/ui/features/correction_wizard/): prefilled from the original entry's fields, `fix()` calls `fixPostedTransaction` (reversal + replacement in one repository transaction). `reverseEntry` rejects an original that already has a reversal.

...
- [x] 6.3 On Android device `SM X230` (`RZGL42CPNGP`, Android 16): live Yahoo resolve + NYSE `UBS` mismatch verified via `flutter test -d RZGL42CPNGP integration_test/instrument_identifier_live_yahoo_android_test.dart` (passed 2026-09-22). The test probes live `searchIdentifier('UBS')` (NYSE USD listing present), drives the real Buy dialog, selects the NYSE listing on the confirm sheet, and asserts holdings `displayQuoteUse == QuoteUse.currencyMismatch` with the "price currency differs" label.

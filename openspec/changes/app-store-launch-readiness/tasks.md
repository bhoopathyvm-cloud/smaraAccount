...
- [ ] 11.3 Retry Validate/Distribute in Xcode Organizer on the same archive now that the app record exists — expected to proceed past `IDEDistributionFetchAppRecordStep` into real validation instead of re-attempting creation; not yet confirmed
- [ ] 11.4 Once Validate passes, upload and complete the rest of section 5/6-equivalent steps for macOS (listing assets, privacy nutrition label, submit for review) — not separately tracked yet since this session's scope only got as far as signing and the app record

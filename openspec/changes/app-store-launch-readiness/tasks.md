...
- [ ] 3.3 Verify an iOS archive build succeeds and is Team-signed — attempted via `flutter build ipa --release` (2026-09-13): failed at the archive step with `Error (Xcode): No Accounts: Add a new account in Accounts settings.` and `No profiles for 'com.smaraaccounting.smaraAccounting' were found`. A Development certificate for the correct team (`OU=PLUT6R5W2W`, matching the project's `DEVELOPMENT_TEAM`) is already in the keychain, but Xcode's own Accounts settings has no Apple ID signed in on this machine, so Automatic signing can't fetch/create a provisioning profile even with a valid cert present. Needs an interactive step: Xcode → Settings → Accounts → add the Apple ID for team PLUT6R5W2W (likely needs 2FA, can't be done headlessly)

...
An investment account's cash also remains a selectable financial account for `multi-account-ledger`'s ordinary income/expense transaction recording, unmodified — this is the escape hatch for a cash-only event Buy/Sell/Dividend/brokerage don't name (a custody fee, an account maintenance fee, a wire fee), without inventing a bespoke action for each one. It never touches inventory.

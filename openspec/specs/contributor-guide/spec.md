...
A discoverable, root-level entry point explaining how to request a
feature, propose and submit a contribution, and where the architecture
and engineering-discipline rules that govern it live. (Purpose derived
from the `project-documentation` change; refine as the capability
evolves.)

## Requirements

### Requirement: Root-Level Contributor Entry Point
The repository SHALL contain a `CONTRIBUTING.md` file at the repository root explaining how to request a feature, propose and submit a contribution: the OpenSpec propose → apply → archive workflow, the branch-per-change convention (one branch per OpenSpec change, named after the change), the rule against committing directly to `main`, and the pull-request submission process. `CONTRIBUTING.md` SHALL link to the existing `Specs/architecture/smara-architecture.md` and `Specs/architecture/smara-tech-guidelines.md` for architecture and engineering-discipline rules rather than restating their content.
...
#### Scenario: Contributor guide explains useful feature requests
- **WHEN** a reader wants to request a feature
- **THEN** `CONTRIBUTING.md` asks them to describe the real problem, the missing or confusing workflow, the desired outcome, and any examples or edge cases
- **AND** it states that using AI tools to refine the issue before submitting is welcome but optional
...
`README.md` SHALL link to `CONTRIBUTING.md` from its "Contributing" section and to `docs/user-guide.md` from a usage-oriented section, so a reader lands on the right document for their goal without needing to browse the repository tree. The "Contributing" section SHALL welcome feature requests as issues, mention that AI tools may be used to clarify a request, and state that work is picked up as time permits.

...
One Flutter-free owner for a pending transfer's resolved settlement target
account and whether the settlement follows the shortfall path, shared by the
settle-pending form (`SettlePendingDraft`) and the ledger write path
(`LedgerPosting.settlePendingTransfer`).

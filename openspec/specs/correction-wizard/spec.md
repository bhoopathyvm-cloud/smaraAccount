...
A guided Fix flow that corrects a posted transaction by pairing a
reversal with a new corrected entry in one sequence, so a mistaken
entry never needs a separate manual reverse-then-re-enter.
...
- **THEN** the system posts a reversal and a new entry with the new category
- **AND** the original entry remains visible

...
Guard the stability of the account management screen's dialogs against
regressions like the dispose-after-dismissal crash this capability was
created to fix. (Purpose derived from the
`account-group-dialog-dispose-race` change; refine as the capability
evolves.)

## Requirements

### Requirement: Account and group dialogs survive dismissal
The account management screen's create/rename dialogs for accounts and groups SHALL remain stable through dismissal — including any in-progress dialog exit transition or keyboard-dismiss animation — without throwing an unhandled exception or triggering Flutter's error screen.

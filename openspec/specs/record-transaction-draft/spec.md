...
Flutter-free draft module for the record-transaction form: split remainder, foreign-currency visibility, paid-from-card/bank filters, and submit readiness — owned outside the ViewModel so rules are unit-testable with `package:test`.

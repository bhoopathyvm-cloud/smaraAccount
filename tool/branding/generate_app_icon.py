...
def pixel(x: int, y: int) -> tuple[int, int, int, int]:
    ...
    if _in_round_rect(x, y, page_l, page_t, page_r, page_b, page_rad):
        # Three evenly-spaced ledger rules, thick enough to survive
        # downscaling.
        for rule_y in (380, 512, 644):
            if abs(y - rule_y) <= 18 and page_l + 80 <= x <= page_r - 80:
                return PRIMARY
        # Remembrance seal in the lower-right of the page.
        if _in_circle(x, y, 680, 740, 42):
            return PRIMARY
        return WHITE
    return PRIMARY
